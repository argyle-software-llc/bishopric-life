...
export default function MembersNeedingCallings() {
  ...
  // Get active members who need callings - split into those with and without callings
  const activeNeedsCallings =
    membersNeedingCallings?.filter((m) => m.status === 'active' && (!m.callings || m.callings.length === 0)) || [];

  // Members who were tagged as needing callings but now have one (ready to be removed)
  const membersWithNewCallings =
    membersNeedingCallings?.filter((m) => m.callings && m.callings.length > 0) || [];

  // Get members on hold
  const onHoldMembers = membersNeedingCallings?.filter((m) => m.status === 'hold' && (!m.callings || m.callings.length === 0)) || [];
  ...
  return (
    <div>
      ...
      {/* Members who now have callings - ready to remove */}
      {membersWithNewCallings.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
            <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Received Calling ({membersWithNewCallings.length})
          </h2>
          <div className="space-y-3">
            {membersWithNewCallings.map((member) => (
              <div
                key={member.id}
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="mt-2">
                      <p className="text-sm font-medium text-green-700">New Calling(s):</p>
                      <ul className="mt-1 space-y-1">
                        {member.callings?.map((calling: any) => (
                          <li key={calling.id} className="text-sm text-green-800 flex items-center gap-1">
                            <span className="font-medium">{calling.title}</span>
                            <span className="text-green-600">- {calling.organization_name}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
